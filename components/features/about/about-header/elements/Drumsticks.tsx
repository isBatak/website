...
function Drumstick() {
  ...
  const { nodes } = useGLTF('/3d/drumstick.glb');
  const [drumstick] = useState(() => nodes[`drumstick`]);
  ...
  return (
    <Float position={position} speed={speed} rotationIntensity={10} floatIntensity={40} dispose={null}>
      <mesh scale={2} geometry={drumstick.geometry} material={material} />
    </Float>
  );
}
...
useGLTF.preload('/3d/drumstick.glb');
